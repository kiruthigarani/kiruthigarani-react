...
export default function AboutUsComponent() {
  ...
  //uplifting state example
  const [name, setName] = useState("Kiruthiga");
 
  const modifiedName = (newName) => {
    console.log("Modified Name in Parent:", newName);
    setName(newName);
  };
  //End of uplifting state example
  return (
    <div className="about-us">
      <h1>About Us</h1>
      <p>
        Welcome to our company. We are dedicated to providing exceptional
        services and solutions to our clients worldwide. With over a decade of
        experience, we have established ourselves as industry leaders committed
      ...
      <p>
        ...
        software development, design, and digital strategy. We work
        collaboratively to bring your vision to life and drive your business
        forward.
      </p>
      ...
      <div>
        ...
        <div
          className={`border-2 border-black m-2 p-2 h-64 ${
            showTheme ? "bg-gray-800" : ""
          }`}
        >
          <button
            className="m-2 p-2 border-2 bg-green-200"
            onClick={() => setShowThem(!showTheme)}
          >
            Toggle
          </button>
          <input
            className="border-2"
            name="heavy_calculattion"
            type="number"
            onChange={(e) => {
              setHeavyCalculation(e.target.value);
            }}
